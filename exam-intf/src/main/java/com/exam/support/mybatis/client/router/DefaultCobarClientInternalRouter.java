...
import com.exam.support.mybatis.client.router.config.DefaultCobarClientInternalRouterXmlFactoryBean;
 ...
 */
public class DefaultCobarClientInternalRouter implements ICobarRouter<RoutingResult,IBatisRoutingFact> {
	...
	private IRoutingRule<IBatisRoutingFact, List<String>> searchMatchedRuleAgainst(
			Set<IRoutingRule<IBatisRoutingFact, List<String>>> rules, IBatisRoutingFact routingFact) {
		if (CollectionUtils.isEmpty(rules)) {
			return null;
		}
