...
var serverIpContext = "";
if(href.indexOf("local") != -1){  //开发本地环境
	context = "http://localhost:8080/exam-web";
	serverIpContext = "http://localhost:8080/exam-web";
} else {
	context = "http://180.76.142.131:8080/exam-web";
	serverIpContext = context
}
...
System.getRequestParams = function(object) {
	var uid = System.cookie.get("boss3VosUserId");
	var nickname="";
	if( System.cookie.get("boss3VosUserName")){
		nickname = Base64.decode( System.cookie.get("boss3VosUserName"));
	}
...
/**
 * data 请求数据 type 请求方式 "get" "post" url 请求服务地址 callback 回调方法,返回数据已解码
 */
System.ajaxSubmitDecode = function(data, type, url, callback) {
	$.ajax({
		// 提交数据的类型 POST GET
		type : type,
		// 提交的网址
		...
		success : function(data) {
			if (callback && typeof callback == "function") {
				callback(System.parseResponseData(data));
			}
		}
	});
};
...
System.packageAjax.request = function(options) {
	...
	return new function() {
		...
		for (var i in this.listener) {
			var functionStr = 'if ("' + i + '" == "success"){' + 'var data = arguments[0], textStatus = arguments[1];' + 'if (this.encode){' + 'try{' + 'data = Base64.decode(data);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行Base64编码处理！请求地址："+this.url+"返回数据："+data);' + '}' + 'try{' + 'data = JSON.parse(data);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行JSON格式检查！请求地址："+this.url+"返回数据："+data);' + '}' + '}else{' + 'data = JSON.parse(data);' + '}' + 'if(!data.success){' + 'alert("后台数据报错：错误代码：" + data.errorCode + "，错误信息：" + data.msg);return;' + '}else{' + 'this.listener["' + i + '"].call(this, data, textStatus);' + '}' + '}else if("' + i + '" == "requestcomplete"){' + 'var data = arguments[0];' + 'try{' + 'data = Base64.decode(data.responseText);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行Base64编码处理！请求地址："+this.url+"返回数据："+data);' + '}' + 'try{' + 'data = JSON.parse(data);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行JSON格式检查！请求地址："+this.url+"返回数据："+data);' + '}' + 'this.listener["' + i + '"].call(this, data, textStatus);' + '}else if("' + i + '" == "beforesuccess"){' + 'var data = arguments[0], type = arguments[1];' + 'this.listener["' + i + '"].call(this, data, type);return data' + '}else if("' + i + '" == "error"){' + 'this.listener["' + i + '"].call(this, arguments);' + '}else{' + 'this.listener["' + i + '"].call(this, arguments);' + '}';
...
//					return Errors[2];
	...
	check: function(container){
		var flag = true,
			title = '';
		$(container, 'input,textarea,select').each(function(i,n){
			title = '';
			var node = $(n),
				value = $(n).val();
			...
			if(title != ''){
				node.addClass('error_input').tooltip({title:title});
			}
		});
		return flag;
	}
...
;(function(win,$){
	...
	JLoading.open = function(msg){
		var html='';
		html += '<div class="loading-mask" role="Tnlonding">';
		html += '<div class="loading-wrapper">';
		html += '<a onclick="$(this).parents(\'[role=Tnlonding]\').remove()" class="loading-close-btn">&times;</a>';
	...
	//延时1秒关闭
	JLoading.close = function(){
		setTimeout(fun,1000);
		function fun(){
			$("[role='Tnlonding']").remove();
		}
	}
	//立即关闭
	JLoading.immediatelyClose = function(){
		$("[role='Tnlonding']").remove();
	}
