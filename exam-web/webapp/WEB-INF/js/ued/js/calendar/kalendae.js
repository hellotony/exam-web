...
Kalendae = function (targetElement, options) {
...
Kalendae.prototype = {
	defaults : {
		...
		futureMaxDay:           null,			/* direction 为 today-future 或者 future， 最大可选日期配置 */
		...
		dateClassMap:			{},
		scrollAdjustDom:        null            /* the element that it's scrollTop will influence the position of container*/
	},
	classes : {
		container		:'kalendae',
		calendar		:'k-calendar',
	...
	//zhouhai:select discrete days
	selectMulDays : function (startDate, endDate, rule) {
		...
		if(days.length > 0){
			this.viewStartDate = moment(days[0],'YYYY-MM-DD');
			this.setSelected(days, true);
		}
	},

	//zhouhai: set visible range, only the days in visible range can be selected
	// setVisibleRange : function(startDate, endDate) {
	// 	if(typeof(startDate) == 'undefined' || typeof(endDate) == 'undefined' || startDate == ''){
	// 		return;
	// 	}
	// 	this.viewStartDate = moment(startDate,'YYYY-MM-DD');
	// 	this.direction = function (date) {return (moment(date).yearDay() <= moment(startDate).yearDay()) || (moment(date).yearDay() >= moment(endDate).yearDay());}
	// 	this.draw();
	// },
	//yelu: set visible range, add the future range and the past range, if you want to range future,just let the endDate be '',past,let the startDate be ''
	setVisibleRange : function(startDate, endDate) {
		...
		this.viewEndDate = moment(endDate,'YYYY-MM-DD');
		this.direction = function (date) {
			return    startDate ? (moment(date).yearDay() <= moment(startDate).yearDay()) : (endDate ? (moment(date).yearDay() > moment(endDate).yearDay()+1) : false);
		}
		this.draw();
	},
	...
	draw : function draw() {
		...
		do {
			...
			do {
				...
				//增加可选日期配置
				else if ((opts.direction==='today-future' || opts.direction==='future') && opts.futureMaxDay && (dateString > opts.futureMaxDay)) klass.push(classes.dayOutOfMonth);

				else if (!(this.blackout(day) || this.direction(day)) || s>0) {
					klass.push(classes.dayActive);
					//if (!(this.select(day))) klass.push(classes.dayInRules);
				}
...
// ELEMENT FUNCTIONS
	...
	getPosition: function (elem, isInner) {
		...
		if(!firefoxAdd){
			//父级是fixed并且不在弹框中
			if(util.isFixed($input) && $($input).parents("#sb_layoutBody").length){
				r[0] = r.left = x;
			}else{
				if (sidebarToggle) {
					r[0] = r.left = x - 52;
...
Kalendae.Input = function (targetElement, options) {
	//zhouhai:just display the calendar and display selected days
	var $input,overwriteInput;
	if(targetElement.nodeName === 'A'){
		$input = this.input = targetElement;
	}else{
		$input = this.input = util.$(targetElement);
		if (!$input || $input.tagName !== 'INPUT') throw "First argument for Kalendae.Input must be an <input> element or a valid element id.";
	}

	//////////////////////////////////////////////////source, noter:zhouhai/////////////////////////////////////////////////////////////////////////////////
	...
	if (opts.attachTo == null) {
		if (firefoxAdd) {
			...
			//没有侧边栏bug修复
			if($("#sb_layoutBody").length){
				opts.attachTo = util.$("sb_layoutBody");
			}else{
				opts.attachTo = window.document.body;
			}
		}
...
Kalendae.Input.prototype = util.merge(Kalendae.prototype, {
	...
	show : function () {
		...
		if(opts.scrollAdjustDom){
			pos[1] = pos.top = pos.top - opts.scrollAdjustDom.scrollTop;
		}
		// 定位bug修复
		if($($input).parents("#sb_layoutBody").length == 0 ){
			if(!firefoxAdd){
				if (sidebarToggle) {
					this.settings.offsetLeft = 52;
				}else{
					this.settings.offsetLeft = 222;
				}
			}
		}
		style.display = '';
